...
public class MeetingJoinRepositoryAdapter implements MeetingJoinRepository {
    ...
    @Override
    public List<MeetingJoin> findByMeetingIdAndJoinStatus(UUID meetingId, MeetingJoinStatus joinStatus) { // 모임+상태 목록 조회 메서드
        return jpaMeetingJoinRepository.findByMeetingIdAndJoinStatus(meetingId, joinStatus);
    }

    @Override
    public Optional<MeetingJoin> findByMeetingIdAndRecruitUserId(UUID meetingId, UUID recruitUserId) { // 모임+유저 단건 조회 메서드
